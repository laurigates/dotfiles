...
class VoiceNotifier:
    ...
    def generate_speech_gemini(self, text: str, voice_name: str, style: str) -> Optional[bytes]:
    ...
    def notify(self, message: str, project: Optional[str] = None, style: Optional[str] = None) -> bool:
        ...
        if GEMINI_AVAILABLE and self.gemini_client:
            cache_key = self.get_cache_key(message, voice_config['voice'], style)
            ...
            if not audio_data:
                # Generate new audio
                audio_data = self.generate_speech_gemini(message, voice_config['voice'], style)
                if audio_data:
