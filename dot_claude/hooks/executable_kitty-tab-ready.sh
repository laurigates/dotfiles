...
# Claude Code Hook: kitty-tab-ready
# Updates kitty tab title to show ready state
...
update_tab_title() {
    ...
    if [[ "${TERM}" == "xterm-kitty" ]]; then
        ...
        if [[ -n "${KITTY_LISTEN_ON:-}" ]]; then
            ...
            kitty @ --to "$KITTY_LISTEN_ON" set-tab-title "$repo_name | ✓ Ready" >/dev/null 2>&1 || true
        else
            # Fall back to OSC escape sequence for tab title
            printf '\033]2;%s\033\\' "$repo_name | ✓ Ready" >/dev/null 2>&1 || true
        fi
    fi
}
